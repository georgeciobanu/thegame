...
- (void)boot
{
	NSLog(@"[INFO] %@/%@ (%s.b357e29)",TI_APPLICATION_NAME,TI_APPLICATION_VERSION,TI_VERSION_STR);
...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kspotonUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kspotonUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
