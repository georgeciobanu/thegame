...
// in simulator we redefine to format for spoton Developer console
...
{\
	NSString * exceptionName = [@"org.spoton." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.spoton";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UIspotonNativeItemNone = -1, 
	UIspotonNativeItemSpinner = -2,
	UIspotonNativeItemProgressBar = -3,
	
	UIspotonNativeItemSlider = -4,
	UIspotonNativeItemSwitch = -5,
	UIspotonNativeItemMultiButton = -6,
	UIspotonNativeItemSegmented = -7,
	
	UIspotonNativeItemTextView = -8,
	UIspotonNativeItemTextField = -9,
	UIspotonNativeItemSearchBar = -10,
	
	UIspotonNativeItemPicker = -11,
	UIspotonNativeItemDatePicker = -12,
	
	UIspotonNativeItemInfoLight = -13,
	UIspotonNativeItemInfoDark = -14,
	
	UIspotonNativeItemDisclosure = -15,
	
	UIspotonNativeItemContactAdd = -16
};
