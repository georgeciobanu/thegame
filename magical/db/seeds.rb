...
# adjacency_list contains the list of neighboring areas for each area
gm = game.create_game_map(name: 'the one', adjacency_list: {1 => [3, 4], 2 => [3, 4], 3 => [1, 2], 4 => [1, 2] })
...
teams = gm.teams.create([{ name: 'Green', color: 'green' }, 
                         { name: 'Blue', color: 'blue' }, 
                         { name: 'Red', color: 'red' }])

Team.find(2).members.create({ name: 'George Ciobanu', email: 'george@lh.com', password: 'password', minion_pool: 2})
